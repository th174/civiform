...
/**
 * A repeated entity represents one of the applicant's answers to an enumerator
 * question.
 */
@AutoValue
public abstract class RepeatedEntity {
  ...
  /**
   * Create all the nested repeated entities associated with the enumerator
   * question, with this
  ...
  /**
   * Helper method to for createRepeatedEntities without a parent.
   * Creates all the non-nested repeated
   * entities associated with the enumerator question.
   */
  ...
  private static ImmutableList<RepeatedEntity> createRepeatedEntities(
      ...
      Optional<PredicateDefinition> visibiity,
      ApplicantData applicantData) {
    Path contextualizedEnumeratorPath = parent
    ...
    ImmutableList<String> entityNames = applicantData.readRepeatedEntities(contextualizedEnumeratorPath);
  ...
  /**
   * The {@link services.question.types.QuestionType#ENUMERATOR} question
  ...
  /**
   * If this is a nested repeated entity, this returns the immediate parent
   * repeated entity.
   */
  ...
  /**
   * The positional index of this repeated entity with respect to the other
   * repeated entities for
  ...
  public Path contextualizedPath() {
    Path parentPath = parent().map(RepeatedEntity::contextualizedPath).orElse(ApplicantData.APPLICANT_PATH);
    return parentPath
        .join(enumeratorQuestionDefinition().getQuestionPathSegment())
        .atIndex(index());
  }
   ...
   *
   * Recursively do the parents FIRST, because {@link String#replace} is
   * eager and will replace
   * "\$this" first and mess up "\$this.parent".
   */
  private String contextualize(String text, String target) {
    String updatedText = parent()
